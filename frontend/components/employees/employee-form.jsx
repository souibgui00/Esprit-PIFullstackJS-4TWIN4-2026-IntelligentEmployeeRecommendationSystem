...
export function EmployeeForm({ employee, mode = "create" }) {
  const { addEmployee, updateEmployee, departments, users, fetchUsers } = useData()
  ...
  // Compute departments that have NO manager assigned.
  // The backend populates dept.manager_id directly, so we check that field.
  const departmentsWithoutManager = (departments || []).filter(d => {
    const mgrId = d.manager_id?._id || d.manager_id?.id || d.manager_id
    const targetId = employee?.id || employee?._id
    // When editing a manager, their own current department should remain available
    if (targetId && mgrId && String(mgrId) === String(targetId)) return true
    // A department is "available" only if it has no manager_id set
    return !mgrId || mgrId === "" || mgrId === null
  })

  // When role changes to HR, clear the department field
  React.useEffect(() => {
    if (role === "hr") {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const payload = {
        ...
        department_id: (data.department_id && data.department_id !== "") ? data.department_id : undefined,
      ...
      if (mode === "create") {
        ...
        if (targetEmployeeId && cvFile) {
          ...
          // Refresh both the employees list and the logged-in user's profile
          // so skills and rankScore are up-to-date immediately
          await Promise.all([fetchUsers(), refreshProfile()])
        }
      ...
      <div className="bg-white border border-slate-100 rounded-[48px] shadow-mega overflow-hidden relative">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-12 space-y-12 relative z-10 bg-white">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Department field: shown for employee & manager, hidden for HR */}
              {role !== "hr" && (
                <div className="space-y-3">
                  <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1 font-mono">
                    Department {role === "manager" && <span className="text-amber-500 ml-1">(Unmanaged Only)</span>}
                  </Label>
                  <Select value={departmentId} onValueChange={(v) => setValue("department_id", v)}>
                    <SelectTrigger className={cn("h-14 bg-slate-50/50 border-slate-100 rounded-2xl text-[11px] font-black tracking-widest uppercase", errors.department_id && "border-rose-300")}>
                      <SelectValue placeholder={role === "manager" ? "SELECT DEPARTMENT (NO MANAGER)" : "SELECT DEPARTMENT"} />
                    </SelectTrigger>
                    <SelectContent className="rounded-2xl border-none shadow-2xl">
                      {(role === "manager" ? departmentsWithoutManager : (departments || [])).map((dept) => (
                        <SelectItem key={dept.id || dept._id} value={dept.id || dept._id} className="text-[10px] font-black uppercase tracking-widest py-3">
                          {dept.name}
                        </SelectItem>
                      ))}
                      {role === "manager" && departmentsWithoutManager.length === 0 && (
