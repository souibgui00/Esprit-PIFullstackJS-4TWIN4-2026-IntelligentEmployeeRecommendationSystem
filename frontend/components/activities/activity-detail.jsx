    ...
    <Card className="border-none shadow-none bg-white rounded-[4px] relative overflow-hidden">
      ...
      <CardHeader className="relative p-10 pb-10 border-b border-[#EEEEEE]">
        ...
        <div className="flex items-start gap-8 pt-4 relative">
          <div className="rounded-[4px] p-6 bg-[#EEEEEE] text-primary shadow-sm">
            <Icon className="h-10 w-10" />
          </div>
          <div className="flex-1 pr-12">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <Badge variant="outline" className="text-[9px] font-bold tracking-widest rounded-xs bg-[#EEEEEE] text-[#222222] border-none px-3 py-1.5 ">
                {getActivityTypeLabel(activity.type).toUpperCase()}
              </Badge>
              <Badge className={cn("text-[9px] font-bold tracking-widest rounded-xs border-none px-3 py-1.5 ",
            ...
            <p className="text-[10px] font-bold text-primary tracking-[.4em] mt-2 shadow-shadow shadow-primary/10">STATUS: {activity.status.toUpperCase()}</p>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-12 p-10">
        ...
        <div className="relative">
          <h4 className="text-[10px] font-bold text-primary tracking-[0.3em] mb-4 ">Description</h4>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="p-6 rounded-[4px] bg-[#F8FAFC] border border-[#EEEEEE] hover:border-primary/20 transition-all">
          ...
          <div className="p-6 rounded-[4px] bg-[#F8FAFC] border border-[#EEEEEE] hover:border-primary/20 transition-all">
            ...
            <p className="text-sm font-bold text-primary tracking-tighter leading-none">{getPriorityLabel(activity.priorityContext)}</p>
          </div>
        </div>
        ...
        <div className="bg-[#EEEEEE]/50 p-8 rounded-[4px] border border-[#EEEEEE]">
          ...
          <div className="h-1.5 w-full bg-[#EEEEEE] rounded-full overflow-hidden">
            <div className="h-full bg-primary transition-all duration-1000 origin-left" style={{ width: `${fillPercentage}%` }}></div>
          </div>
          <div className="mt-4 flex justify-between items-center">
            <p className="text-[9px] text-gray-400 font-bold tracking-[0.2em] ">
              {seatsRemaining} operational slots identified
            </p>
            <span className="text-[9px] font-bold text-[#222222] bg-white px-3 py-1 rounded-xs">{Math.round(fillPercentage)}% LOAD</span>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {activity.requiredSkills.map((rs) => (
              <div
                key={rs.skillId}
                className="flex items-center justify-between p-5 rounded-[4px] bg-white border border-[#EEEEEE] shadow-sm hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all group"
              >
                <div>
                  <p className="text-sm font-bold text-[#222222] tracking-tighter group-hover:text-primary transition-colors">{rs.skill?.name || 'Skill Point'}</p>
                ...
                <div className="flex items-center gap-6">
                  <div className="flex flex-col items-end">
                    ...
                    <Badge className={cn("text-[9px] font-bold tracking-widest rounded-xs border-none px-3 py-1 ", getSkillLevelColor(rs.requiredLevel))}>
                      {rs.requiredLevel}
                    </Badge>
                  </div>
                  <div className="h-10 w-px bg-[#EEEEEE]"></div>
                  <div className="flex flex-col items-end">
                    <p className="text-[8px] font-bold text-gray-300 tracking-widest mb-1">WEIGHT</p>
                    <span className="text-[10px] font-bold text-[#222222] tracking-widest bg-[#EEEEEE] px-2 py-1 rounded-xs">
                      {rs.weight}
                    </span>
                  </div>
                </div>
