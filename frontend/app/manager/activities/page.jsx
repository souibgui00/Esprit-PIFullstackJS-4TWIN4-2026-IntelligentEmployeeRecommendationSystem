...
export default function ManagerActivitiesPage() {
  ...
  const managerDept = departments.find(d => {
    ...
    return mId && userIdStr && mId.toString() === userIdStr
  })
  const uDeptId = getDeptId(managerDept) || getDeptId(user?.department_id) || getDeptId(user?.department)
  const uDeptName = getDeptName(managerDept) || getDeptName(user?.department_id) || getDeptName(user?.department) || user?.department || "Unassigned"
