...
export function DataProvider({ children }) {
      ...
      if (Array.isArray(data)) {
        ...
        const mappedEmployees = data.map(u => {
          ...
          return {
            ...
            _id: id,
            userId: id,
            // Preserve the raw department ObjectId string so all department-id comparisons work.
            // When the backend populates department_id it becomes {_id, name, code}; extract the id.
            department_id: u.department_id && typeof u.department_id === 'object'
              ? (u.department_id._id || u.department_id.id || u.department_id).toString()
              : u.department_id
                ? u.department_id.toString()
                : null,
            // Separate readable display field
            departmentName: (u.department_id && typeof u.department_id === 'object'
              ? u.department_id.name
              : null) || u.department || null,
            department: (u.department_id && typeof u.department_id === 'object'
              ? u.department_id.name
              : null) || u.department || u.department_id || null,
            position: u.role,
            avatar: u.avatar,
  ...
  const addActivity = useCallback(async (activityData) => {
    try {
      const newActivity = await api.post("/api/activities", activityData)
  ...
  const updateActivity = useCallback(async (id, data) => {
    try {
      ...
      await api.put(`/api/activities/${id}`, data)
      await fetchActivities()
