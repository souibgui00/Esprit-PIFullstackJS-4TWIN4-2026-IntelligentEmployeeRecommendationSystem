...
class ApiService {
    ...
    async upload(endpoint, fileOrFormData, retries = 0) {
        const token = sessionStorage.getItem("skillmatch_token") || localStorage.getItem("token")
        let body;
        ...
        try {
            ...
            if (response.status === 401 && retries === 0) {
                const refreshToken = sessionStorage.getItem("skillmatch_refresh_token") || localStorage.getItem("refresh_token")
                if (refreshToken) {
                    try {
                        const refreshResponse = await fetch(`${API_URL}/auth/refresh`, {
                            method: "POST",
                        ...
                        if (refreshResponse.ok) {
                            ...
                            sessionStorage.setItem("skillmatch_token", access_token)
                            if (refresh_token) sessionStorage.setItem("skillmatch_refresh_token", refresh_token)
                            return this.upload(endpoint, fileOrFormData, 1)
                        }
                    } catch (e) {
                        console.error("[API] Upload refresh failed", e)
                    }
                }
            }
